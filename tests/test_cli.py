...
""".strip()
        blockers = cli.parse_lsof_output(raw)
    ...
    def test_parse_tmutil_status(self) -> None:
        ...
        parsed = cli.parse_tmutil_status(sample)
...
""".strip()
        mounts = cli.parse_tmutil_destinationinfo(raw)
...
class TMReportTests(unittest.TestCase):
    ...
    def _complete(cmd, *, stdout="", stderr="", returncode=0):
        return cli.subprocess.CompletedProcess(cmd, returncode=returncode, stdout=stdout, stderr=stderr)
    ...
    def test_run_diagnostic_smoke(self) -> None:
        ...
        report = cli.run_diagnostic(target, command_runner=fake_runner)
    ...
    def test_json_render(self) -> None:
        ...
        namespace = cli.argparse.Namespace(target=target, json=True, stop_backup=False, wait=0)
        with mock.patch("builtins.print"):
            rc = cli._run(namespace)
