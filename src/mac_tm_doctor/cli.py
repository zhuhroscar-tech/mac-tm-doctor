...
def run_diagnostic(target: str, command_runner=run_command) -> Dict[str, object]:
    ...
    if blockers:
        pids = {b["pid"] for b in report["blockers"]}
        top = sorted(pids)[:5]
...
def _render_report(report: Dict[str, object], *, include_json: bool = False) -> str:
    target = report["target"]
    blockers = report["blockers"]  # type: ignore[assignment]
    tm_info = report.get("timeMachine", {})  # type: ignore[assignment]
    suggestions = report.get("suggestions", [])  # type: ignore[assignment]

    lines: List[str] = []
    lines.append(f"mac-tm-doctor report for: {target}")
    lines.append("-" * 60)
...
def _run(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        if not report["blockers"] and not report["timeMachine"]["status"].get("running"):
            return 0
